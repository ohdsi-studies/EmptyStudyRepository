...
( 
  select concept_id from dbo.CONCEPT where concept_id in (255841,255573,4279071)
UNION  select c.concept_id
  from dbo.CONCEPT c
  join dbo.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (255841,255573,4279071)
...
DELETE FROM results.cohort where cohort_definition_id = 658;
INSERT INTO results.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
select 658 as cohort_definition_id, person_id, start_date, end_date 
...
SELECT 0 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from dbo.CONCEPT where concept_id in (255573,258780)
UNION  select c.concept_id
  from dbo.CONCEPT c
  join dbo.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from dbo.CONCEPT where concept_id in (1112921,1106776)
UNION  select c.concept_id
  from dbo.CONCEPT c
  join dbo.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
( 
  select concept_id from dbo.CONCEPT where concept_id in (317009,4235703,4279553)
UNION  select c.concept_id
  from dbo.CONCEPT c
  join dbo.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
...
JOIN dbo.PERSON P on C.person_id = P.person_id
...
DELETE FROM results.cohort where cohort_definition_id = 152;
INSERT INTO results.cohort (cohort_definition_id, subject_id, cohort_start_date, cohort_end_date)
select 152 as cohort_definition_id, person_id, start_date, end_date 
FROM #final_cohort CO
;
...
select top 1000 1000, subject_id, cohort_start_date, cohort_end_date
from results.cohort
where cohort_definition_id = 152 and cohort_start_date>'2005-01-01';